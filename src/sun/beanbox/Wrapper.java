...
public class Wrapper extends Panel implements Serializable, MouseListener,
    ...
    public Wrapper(Object bean, String beanLabel, String beanName, String jarPath) {
        ...
        initialize();
        listenerBeans = new Hashtable<String,Vector>(); //hv2016
        attachListeners();
    }
    ...
    synchronized void addEventTarget(String eventSetName,
        ...
        if (eventSetName.equals("propertyChange"))
            addBeanListener(listener,"propertyChange");
...
//1. Event Listeners
        if (HookupManager.useEventAdapter)  {   //hv2016
        ...
        else { //begin hv2016: case direct event hookup
            ...
            try {
                ...
                for (int i = 0; i < esd.length; i++) {
                    Method removeMethod = esd[i].getRemoveListenerMethod();
                    if (listenerBeans.containsKey(esd[i].getName())&& !esd[i].getName().equals("propertyChange") )  {
                        for (java.util.Iterator it = listenerBeans.get(esd[i].getName()).iterator(); it.hasNext();) {
                            Object target = it.next();
                            removeMethod.invoke(bean, new Object[]{target});
                        }
                    }
                }
...
//2. hv2016: Propertychangelisteners: independent from direct event hookup
        Object bean = this.bean;
    ...
    //	private Hashtable esdMap;  //hv2016
...
//	private Vector changedProperties;  //hv2016
    ...
    public void addBeanListener(Object target, String name) {
        // hv2016: add a listener bean target for an event set "name"
        if (listenerBeans.containsKey(name))  {
            Vector listeners = listenerBeans.get(name);
    ...
    public List<WrapperPropertyEventInfo> getPropertyTargets() {
        List<WrapperPropertyEventInfo> listeners = new ArrayList<>();
        for (Object propertyTarget : propertyTargets) {
            WrapperPropertyEventInfo target = (WrapperPropertyEventInfo) propertyTarget;
            if (target.targetBean != null) {
                listeners.add(target);
